...
async function verifyOnDutyInserted() {
  ...
  try {
    ...
    for (const record of odData) {
      ...
      if (existing) {
        console.log(`[VERIFIED] ${record.rawEmpId} (${user.name}) on date ${record.date} exists in DB (ID: ${existing._id})`);
      } else {
        console.error(`[FAIL]     No record found for ${record.rawEmpId} (${user.name}) on date ${record.date}`);
        allVerified = false;
      }
    }

    console.log(`\nFinal Verification Result: ${allVerified ? "SUCCESS - All 31 records verified in database!" : "FAILURE - Some records are missing!"}`);
